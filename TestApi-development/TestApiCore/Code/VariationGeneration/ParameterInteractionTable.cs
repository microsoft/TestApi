...
namespace Microsoft.Test.VariationGeneration
{
    ...
    /// </summary>
    internal class ParameterInteractionTable
    {
        public ParameterInteractionTable(Model model, int order)
        {
            GenerateInteractionsForParameters(model.Parameters, order);
        ...
        public IEnumerable<ValueCombination> ExcludedCombinations()
        {
            return from interaction in Interactions
        ...
        private void GenerateInteractionsForConstraints(Model model, int order)
        {
        ...
        private void GenerateDependentConstraintInteractions(Model model, int order, List<ParameterInteraction> constraintInteractions)
        {
            ...
            foreach (var dependentConstraintSet in dependentConstraintSets)
            {
                ...
                var uniqueParameters = new HashSet<int>();
                var parameterInteractionCounts = new HashSet<int>();
                for (int i = 0; i < dependentConstraintSet.Count; i++)
                {
                    uniqueParameters.UnionWith(dependentConstraintSet[i].Parameters);
                ...
                var sortedParameters = uniqueParameters.ToList();
                ...
                var completeInteractionExcludedCombinations =
                    from combination in completeInteraction.Combinations
                    from constraint in model.Constraints
                    where constraint.SatisfiesContraint(model, combination) == ConstraintSatisfaction.Unsatisfied
                    select combination;

                foreach (var combination in completeInteractionExcludedCombinations)
                {
                    combination.State = ValueCombinationState.Excluded;
                }
                ...
                var allowedCombinations = completeInteraction.Combinations
                    .Where((c) => c.State != ValueCombinationState.Excluded);
                ...
                var individualInteractionExcludedCombinations =
                    from interaction in Interactions
                    from combination in interaction.Combinations
                    where !combination.ParameterToVaueMap.Any((p) => !completeInteraction.Parameters.Contains(p.Key)) 
        ...
        private void GenerateHigherOrderDependentExclusions(Model model, int order, HashSet<int> parameterInteractionCounts, ParameterInteraction completeInteraction, IEnumerable<ValueCombination> allowedCombinations)
        {
            ...
            foreach (var count in parameterInteractionCounts)
            {
                ...
                foreach (var combinations in parameterCombinations)
                {
                    ...
                    var possibleValues = ParameterInteractionTable.GenerateValueTable(model.Parameters, interaction);
        ...
        private void GenerateInteractionsForParameters(IList<Parameter> parameters, int order)
        {
            IList<int[]> parameterCombinations = GenerateCombinations(parameters.Count, order);
            ...
            GenerateValueCombinations(parameters);
        }

        // create all the values for each combination
        private void GenerateValueCombinations(IList<Parameter> parameters)
        {
            foreach (var interaction in Interactions)
            {
                var valueTable = GenerateValueTable(parameters, interaction);

                foreach (var value in valueTable)
                {
                    interaction.Combinations.Add(new ValueCombination(value, interaction));
                }
            }
        }
        ...
        private static IList<int[]> GenerateCombinations(int totalElements, int combinationSize)
        {
            ...
            int totalCombinations = Factorial(totalElements) / (Factorial(combinationSize) * Factorial(totalElements - combinationSize));

            var combinations = new List<int[]>(totalCombinations);
