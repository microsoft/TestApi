...
namespace Microsoft.Test
{
    ...
    */
    ...
    public class CommandLineDictionary : Dictionary<string, string>
    {
        ...
        public CommandLineDictionary(IEnumerable<string> arguments)
            : this(arguments, '/', '=')
        {
        }
        ...
        /// Supports the ability to specify delimiter characters for options in the command line.
        /// </summary>
        /// <param name="arguments">Key/value pairs.</param>
        /// <param name="keyCharacter">Character that precedes a key.</param>
        /// <param name="valueCharacter">Character that separates a key from a value.</param>
        public CommandLineDictionary(IEnumerable<string> arguments, char keyCharacter, char valueCharacter)
        ...
        public override string ToString()
        {
            ...
            foreach (KeyValuePair<string, string> pair in this)
            {
                ...
                {
                    commandline += string.Format(CultureInfo.InvariantCulture, "/{0}={1} ", pair.Key, pair.Value);
                }
                else // There is no value, so we just serialize the key
                {
                    commandline += string.Format(CultureInfo.InvariantCulture, "/{0} ", pair.Key);
                }
            }
