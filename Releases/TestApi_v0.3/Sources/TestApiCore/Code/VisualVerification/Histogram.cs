...
namespace Microsoft.Test.VisualVerification
{
    ...
    public class Histogram
    {
        ...
        public static Histogram FromSnapshot(Snapshot snapshot)
        {
            ...
            float contributionPerPixel = 1 / (float)(snapshot.Width * snapshot.Height);
        ...
        public static Histogram FromFile(string filePath)
        {
            ...
            SortedDictionary<byte, float> loadedTable = new SortedDictionary<byte, float>();
            for (int t = 0; t < nodeList.Count; t++)
            {
                byte x = byte.Parse(nodeList[t].Attributes["x"].InnerText, NumberFormatInfo.InvariantInfo);
                float y = float.Parse(nodeList[t].Attributes["y"].InnerText, NumberFormatInfo.InvariantInfo);
                VerifyPoint(x, y);
                loadedTable[x] = y;
            }
        ...
        public float this[int column] { get { return graph[column]; } }
        ...
        private void InterpolatePoints(SortedDictionary<byte, float> loadTable)
        {
            ...
            foreach (byte curr in loadTable.Keys)
            {
                ...
                if (0 != curr)
                {
                    ...
                    {
                        graph[i] = LinearInterpolate((float)(curr - prev) / (i - prev), loadTable[prev], loadTable[curr]);
                    }
                }
        ...
        /// <returns></returns>
        private static float LinearInterpolate(float proportion, float startValue, float endValue)
        {
            return startValue * (1 - proportion) + endValue * proportion;
        }
        ...
        private static void VerifyPoint(byte x, float y)
        {
