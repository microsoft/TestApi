...
namespace Microsoft.Test.Text
{
    ...
    internal class SurrogatePairProperty : IStringProperty
    {
        ...
        /// </summary>
        public SurrogatePairProperty(UnicodeRangeDatabase unicodeDb, UnicodeRange expectedRange)
        {
            bool isValid = false;
            ...
            surrogateRange = RangePropertyCollector.GetRange(new UnicodeRange(0x10000, TextUtil.MaxUnicodePoint), expectedRange);
            ...
            if (!isValid)
            {
                throw new ArgumentOutOfRangeException("SurrogatePairProperty, SurrogatePair ranges are beyond expected range, " + 
                    String.Format(CultureInfo.InvariantCulture, "0x{0:X}", expectedRange.StartOfUnicodeRange) + " - " + 
                    String.Format(CultureInfo.InvariantCulture,"0x{0:X}", expectedRange.EndOfUnicodeRange) + ". Refert to Surrogates range and UTF32.");
            }
        }
