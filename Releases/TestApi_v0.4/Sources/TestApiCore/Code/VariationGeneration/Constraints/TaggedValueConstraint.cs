...
namespace Microsoft.Test.VariationGeneration.Constraints
{
    ...
    /// </summary>
    internal class TaggedValueConstraint : Constraint
    {
        internal override ParameterInteraction GetExcludedCombinations(Model model)
        {
            if (CachedInteraction == null)
            {
                ...
                var combinationIndices = ParameterInteractionTable.GenerateValueTable(model.Parameters, CachedInteraction);
        ...
        private static IList<ParamaterAndValueIndices> GetParametersWithTaggedValues(Model model)
        {
            ...
            for (int i = 0; i < model.Parameters.Count; i++)
            {
                ...
                {
                    if(IsTagged(model.Parameters[i][j], model.DefaultVariationTag))
                    {
                        if(index == null)
                        {
        ...
        {
            return value is ParameterValue &&
                ((ParameterValue)value).Tag != null &&
                ((ParameterValue)value).Tag != defaultTag;
        }
